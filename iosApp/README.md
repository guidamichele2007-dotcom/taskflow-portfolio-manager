...
**Stato** (Sprint 5, Macro Sprint "MVP Vertical Slice"): `RootView` ora assembla la shell a 4 tab (`AppTab`: Oggi/Moduli/Cerca/Profilo, Navigation Bible §3). Solo "Oggi" ha uno schermo reale (`TaskListPlaceholderView`, mirror SwiftUI di `feature-task`'s `TaskListScreen` Compose) — gli altri tre tab restano `ComingSoonView`, un placeholder onesto, mai contenuto inventato. `TaskListPlaceholderView` usa un modello Swift locale (`TaskRowModel`) invece di consumare direttamente il framework `:shared` — il punto di integrazione con lo `StateFlow` esportato da Kotlin è documentato nel file stesso ma non scritto, perché richiede l'header Objective-C generato dalla compilazione reale del framework iOS di `:shared` (vedi nota sotto).
