...
/**
 * [FiniteAnimationSpec]s for the 4 UX Bible MUC §4 durations/curves, each with the
...
public object OmniMotionSpecs {
    public fun <T> micro(reduceMotion: Boolean): FiniteAnimationSpec<T> =
        if (reduceMotion) snap() else tween(durationMillis = OmniMotionDurationMs.MICRO, easing = LinearOutSlowInEasing)

    public fun <T> standard(reduceMotion: Boolean): FiniteAnimationSpec<T> =
        if (reduceMotion) snap() else spring(dampingRatio = 0.86f, stiffness = Spring.StiffnessMediumLow)

    public fun <T> enfasi(reduceMotion: Boolean): FiniteAnimationSpec<T> =
        if (reduceMotion) snap() else spring(dampingRatio = 0.75f, stiffness = Spring.StiffnessMedium)

    public fun <T> uscita(reduceMotion: Boolean): FiniteAnimationSpec<T> =
        if (reduceMotion) {
            snap()
        } else {
            tween(
                durationMillis = OmniMotionDurationMs.USCITA,
                easing = FastOutLinearInEasing,
            )
        }

    /** motion.progressione (Design System Bible §03 §3): interpolated, never a jump straight to the final value. */
    public fun <T> progressione(reduceMotion: Boolean): FiniteAnimationSpec<T> =
        if (reduceMotion) {
            snap()
        } else {
            tween(
                durationMillis = OmniMotionDurationMs.STANDARD,
