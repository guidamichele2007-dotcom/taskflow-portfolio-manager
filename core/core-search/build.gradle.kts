...
// Same gating as domain/domain-task/build.gradle.kts — only this module's
// SQLDelight driver dependency needs it (see build-logic/build.gradle.kts
// for why the convention plugin itself avoids a compile-time AGP dependency).
...
sqldelight {
    databases {
        create("SearchDatabase") {
            packageName.set("com.omnilife.core.search.persistence")
        }
    }
}
