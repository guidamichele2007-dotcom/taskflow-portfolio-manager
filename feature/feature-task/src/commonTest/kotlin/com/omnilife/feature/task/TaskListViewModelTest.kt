...
class TaskListViewModelTest {
    ...
    @Test
    fun `loads the today view on creation`() =
        runTest {
    ...
    @Test
    fun `changing mode reloads the list for the new view`() =
        runTest {
            val scope = CoroutineScope(UnconfinedTestDispatcher())
            val repository =
                FakeTaskRepository().apply {
    ...
    @Test
    fun `completing a task with open subtasks surfaces a pending choice instead of failing silently`() =
        runTest {
            val scope = CoroutineScope(UnconfinedTestDispatcher())
            val repository =
                FakeTaskRepository().apply {
    ...
    @Test
    fun `resolving the pending subtask choice completes the task and clears the pending state`() =
        runTest {
            val scope = CoroutineScope(UnconfinedTestDispatcher())
            val repository =
                FakeTaskRepository().apply {
    ...
    @Test
    fun `search updates the query and the visible tasks`() =
        runTest {
            val scope = CoroutineScope(UnconfinedTestDispatcher())
            val repository =
                FakeTaskRepository().apply {
    ...
    @Test
    fun `clearing the search query restores the current view`() =
        runTest {
            val scope = CoroutineScope(UnconfinedTestDispatcher())
            val repository =
                FakeTaskRepository().apply {
    ...
    @Test
    fun `postpone reschedules and refreshes the view`() =
        runTest {
            val scope = CoroutineScope(UnconfinedTestDispatcher())
            val repository =
                FakeTaskRepository().apply {
    ...
    @Test
    fun `an unknown task error is surfaced on the state`() =
        runTest {
            val scope = CoroutineScope(UnconfinedTestDispatcher())
