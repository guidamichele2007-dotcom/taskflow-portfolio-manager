...
class DeleteTaskTest {
    @Test
    fun `delete moves the task to trash without removing it (MFC-R-09)`() = runTest {
    ...
    @Test
    fun `restore returns a trashed task to active with no trashedAt`() = runTest {
    ...
    @Test
    fun `permanently deleting a task also removes its subtasks`() = runTest {
    ...
    @Test
    fun `deleting an unknown task returns TaskNotFound`() = runTest {
