...
class CompleteTaskTest {
    ...
    @Test
    fun `TASK-AC-01 completing a weekly recurring task generates exactly one next occurrence`() = runTest {
    ...
    @Test
    fun `completing a non-recurring task never creates a new occurrence`() = runTest {
    ...
    @Test
    fun `TASK-AC-03 completing a task with open subtasks requires an explicit choice`() = runTest {
    ...
    @Test
    fun `resolving the subtask choice with true completes every open subtask`() = runTest {
        val task = Task(envelope = testEnvelope("task-1"), title = "With subtasks", listId = "list-1")
        val repository = repositoryWith(task)
        repository.subtasks["sub-1"] = Subtask(id = "sub-1", taskId = "task-1", title = "Step 1")
        val completeTask = CompleteTask(repository, InMemoryEventBus(), newId = { "task-2" })
    ...
    @Test
    fun `resolving the subtask choice with false completes the task but leaves subtasks open`() = runTest {
        val task = Task(envelope = testEnvelope("task-1"), title = "With subtasks", listId = "list-1")
        val repository = repositoryWith(task)
        repository.subtasks["sub-1"] = Subtask(id = "sub-1", taskId = "task-1", title = "Step 1")
        val completeTask = CompleteTask(repository, InMemoryEventBus(), newId = { "task-2" })
    ...
    @Test
    fun `completing an already-completed task is a no-op success`() = runTest {
        val task = Task(
            envelope = testEnvelope("task-1"),
            title = "Already done",
            listId = "list-1",
    ...
    @Test
    fun `completing an unknown task returns TaskNotFound`() = runTest {
        val repository = FakeTaskRepository()
        val completeTask = CompleteTask(repository, InMemoryEventBus(), newId = { "task-2" })
