...
class PostponeTaskTest {
    ...
    @Test
    fun `postpone to tomorrow moves the due date by exactly one day`() =
        runTest {
    ...
    @Test
    fun `postpone to tonight sets the due date to today`() =
        runTest {
            val task = Task(envelope = testEnvelope("task-1"), title = "T", listId = "list-1")
            val repository = FakeTaskRepository().apply { tasks[task.envelope.id] = task }
            val postponeTask = PostponeTask(repository, InMemoryEventBus(), fixedClock, utc)
    ...
    @Test
    fun `postpone to weekend lands on the next Saturday`() =
        runTest {
            val task = Task(envelope = testEnvelope("task-1"), title = "T", listId = "list-1")
            val repository = FakeTaskRepository().apply { tasks[task.envelope.id] = task }
            val postponeTask = PostponeTask(repository, InMemoryEventBus(), fixedClock, utc)
    ...
    @Test
    fun `postpone to a specific date sets it verbatim`() =
        runTest {
            val task = Task(envelope = testEnvelope("task-1"), title = "T", listId = "list-1")
            val repository = FakeTaskRepository().apply { tasks[task.envelope.id] = task }
            val postponeTask = PostponeTask(repository, InMemoryEventBus(), fixedClock, utc)
