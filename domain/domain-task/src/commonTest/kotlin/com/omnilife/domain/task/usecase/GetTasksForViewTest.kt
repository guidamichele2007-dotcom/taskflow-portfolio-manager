...
class GetTasksForViewTest {
    ...
    @Test
    fun `TASK-014 overdue and today are never mixed`() =
        runTest {
            val repository =
    ...
    @Test
    fun `upcoming view only shows tasks due strictly after today`() =
        runTest {
            val repository =
                FakeTaskRepository().apply {
                    tasks["today"] = task("today", LocalDate(2026, 7, 21))
    ...
    @Test
    fun `a completed overdue task never appears in the overdue view`() =
        runTest {
            val repository =
                FakeTaskRepository().apply {
    ...
    @Test
    fun `all view returns every active task regardless of date`() =
        runTest {
            val repository =
                FakeTaskRepository().apply {
                    tasks["a"] = task("a", null)
