...
class SearchTasksTest {
    @Test
    fun `finds tasks whose title matches the query`() = runTest {
    ...
    @Test
    fun `finds tasks whose notes match the query`() = runTest {
        val repository = FakeTaskRepository().apply {
    ...
    @Test
    fun `a blank query returns no results rather than everything`() = runTest {
        val repository = FakeTaskRepository().apply {
