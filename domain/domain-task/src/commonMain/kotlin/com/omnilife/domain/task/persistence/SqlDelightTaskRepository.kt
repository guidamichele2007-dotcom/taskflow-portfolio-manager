...
public class SqlDelightTaskRepository(driver: SqlDriver) : TaskRepository {
    ...
    override suspend fun insertTask(task: Task): Unit = withContext(Dispatchers.Default) {
        val row = task.toRow()
    ...
    override suspend fun findTaskById(id: EntityId): Task? = withContext(Dispatchers.Default) {
        queries.selectTaskById(id).executeAsOneOrNull()?.toDomain()
    }

    override suspend fun findTasks(filter: TaskFilter, sort: TaskSort): List<Task> = withContext(Dispatchers.Default) {
        val rows = when {
            filter.listId != null -> queries.selectTasksByLifecycleAndList(filter.lifecycleState.name, filter.listId)
                .executeAsList()

            filter.priority != null -> queries
    ...
    override suspend fun permanentlyDeleteTask(id: EntityId): Unit = withContext(Dispatchers.Default) {
        queries.deleteTaskPermanently(id)
    }

    override suspend fun findSubtasks(taskId: EntityId): List<Subtask> = withContext(Dispatchers.Default) {
        queries.selectSubtasksByTaskId(taskId).executeAsList().map { it.toDomain() }
    }

    override suspend fun insertSubtask(subtask: Subtask): Unit = withContext(Dispatchers.Default) {
    ...
    override suspend fun deleteSubtask(id: EntityId): Unit = withContext(Dispatchers.Default) {
        queries.deleteSubtaskById(id)
    }

    override suspend fun permanentlyDeleteSubtasksForTask(taskId: EntityId): Unit = withContext(Dispatchers.Default) {
        queries.deleteSubtasksByTaskId(taskId)
    }

    override suspend fun findListById(id: EntityId): TaskList? = withContext(Dispatchers.Default) {
        queries.selectTaskListById(id).executeAsOneOrNull()?.toDomain()
    }

    override suspend fun findAllLists(): List<TaskList> = withContext(Dispatchers.Default) {
        queries.selectAllTaskLists().executeAsList().map { it.toDomain() }
    }

    override suspend fun insertList(list: TaskList): Unit = withContext(Dispatchers.Default) {
        val row = list.toRow()
    ...
    private fun applySort(tasks: List<Task>, sort: TaskSort): List<Task> = when (sort) {
        TaskSort.DEFAULT -> tasks.sortedWith(
