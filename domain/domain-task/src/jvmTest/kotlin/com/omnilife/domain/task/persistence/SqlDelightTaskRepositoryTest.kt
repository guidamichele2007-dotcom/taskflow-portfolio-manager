...
class SqlDelightTaskRepositoryTest {
    ...
    private fun envelope(id: String) = Envelope(
        id = id,
    ...
    @Test
    fun `a task round-trips through insert and findById unchanged`() = runTest {
        val task = Task(
    ...
    @Test
    fun `updating a task overwrites the stored row`() = runTest {
    ...
    @Test
    fun `findTasks filters by lifecycle state`() = runTest {
    ...
    @Test
    fun `findTasks filters by list id`() = runTest {
    ...
    @Test
    fun `searchTasks matches on title and notes, case-insensitively`() = runTest {
    ...
    @Test
    fun `permanentlyDeleteTask removes the row entirely`() = runTest {
        repository.insertTask(Task(envelope = envelope("a"), title = "A", listId = "list-1"))
    ...
    @Test
    fun `subtasks round-trip and are ordered`() = runTest {
    ...
    @Test
    fun `deleting a task's subtasks removes only that task's rows`() = runTest {
    ...
    @Test
    fun `task lists round-trip through insert and findById`() = runTest {
