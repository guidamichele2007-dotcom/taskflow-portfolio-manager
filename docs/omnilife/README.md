...
| — | [Technology Decision Record](technology_decision_record.md) | Le 18 decisioni tecnologiche approvate (linguaggio, backend, database, sync, sicurezza, DevOps…) più le voci TDR-19…28 aggiunte durante gli sprint, ciascuna con almeno 3 alternative confrontate |
...
| — | [Sprint 3 Report — Core Sync Engine](sprint3_report.md) | Il motore di sincronizzazione offline-first condiviso: CRDT Engine, Conflict Resolver, Merge Engine, Version Manager, Change Tracker, Delta Generator, Local Change Queue persistente (SQLDelight), Retry Engine, Sync Scheduler, Network Monitor, Sync State Manager, Background Sync (`core-sync`, TDR-24/27/28) — più `core-security` (TDR-23) e `core-search` (TDR-25) completati prima del raffinamento di perimetro; `core-notifications` descoped |
