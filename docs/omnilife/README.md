...
| — | [Technology Decision Record](technology_decision_record.md) | Le 18 decisioni tecnologiche approvate (linguaggio, backend, database, sync, sicurezza, DevOps…) più le voci TDR-19…46 aggiunte durante gli sprint, ciascuna con almeno 3 alternative confrontate |
...
| — | [Sprint 5 Report — MVP Vertical Slice](sprint5_report.md) | La prima fetta verticale realmente usabile: `domain-account` (Setting/Onboarding, TDR-36), i tre bridge Task↔Core (ricerca/notifiche/sync outbox), Task Flow completo (crea/modifica/completa/posticipa), Home con dati reali, Onboarding, Impostazioni e Ricerca globale (nuovi moduli `feature-settings`/`feature-search`), navigazione a 4 tab scritta a mano (TDR-38) — più TDR-34…40 e la correzione del leak di `SyncStateManager.observe` già individuato nello Sprint 4 |
| — | [Sprint 6 Report — MVP Hardening + Real Device Readiness](sprint6_report.md) | Audit del codice reale (non della documentazione) di ogni area dell'MVP: le notifiche non arrivavano mai alla tray di sistema Android (corretto — `NotificationFireReceiver`, TDR-42), `RestoreTask` non aveva eventi né UI (corretto, TDR-45), tema/accento non reattivi (corretto, TDR-43), stato perso al cambio tab e 3 `runBlocking` sul thread principale (corretti), varianti di build Debug/Internal/Release (TDR-46) — più `docs/omnilife/android_mvp_test_plan.md`/`ios_mvp_test_plan.md`, checklist riproducibili per ciò che questo sandbox non può verificare, e classificazione GREEN/YELLOW/RED per ogni area |
| — | [MVP Release 1.0 Report](mvp_release_report.md) | Chiude i gap ancora aperti dopo lo Sprint 6: icona/adaptive icon/splash screen reali (`OmniAccent.BLU`, non un'invenzione di brand), sopravvivenza dei promemoria al riavvio del dispositivo (`BootCompletedReceiver`), deep link reale dalla notifica al task specifico, sovrapposizione FAB/snackbar corretta, test di integrazione ricerca senza ghost result (FTS5 reale) e di persistenza su file SQLite reale attraverso una riconnessione — 319 test `jvmTest`, 0 fallimenti; classificazione GREEN/YELLOW/RED, scope MVP e post-MVP espliciti |
